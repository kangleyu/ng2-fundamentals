...
export class EventService {
  ...
  public getEvents(): Observable<IEvent[]> {
    return this.http.get("http://localhost:8809/api/events").map((response: Response )=> {
      return response.json() as IEvent[];
    }).catch(this.handleError);
  }

  public getEvent(id: number): Observable<IEvent> {
    return this.http.get("http://localhost:8809/api/events/" + id).map((response: Response) => {
      return response.json() as IEvent;
    }).catch(this.handleError);
  }
  ...
  public saveEvent(event): Observable<IEvent> {
    ...
    return this.http.post("http://localhost:8809/api/events", JSON.stringify(event), options)
  ...
  public searchSessions(searchTerm: string) {
    return this.http.get("http://localhost:8809/api/sessions/search?search=" + searchTerm).map((response: Response )=> {
      return response.json();
    }).catch(this.handleError);
  }
